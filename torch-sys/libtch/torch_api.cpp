...
tensor at_tensor_of_data(void *vs, int64_t *dims, size_t ndims, size_t element_size_in_bytes, int type)
{
  PROTECT(
      ...
      if ((int64_t)element_size_in_bytes != tensor.element_size()) throw std::invalid_argument("incoherent element sizes in bytes");
...
{
  PROTECT(
      if ((int64_t)elt_size_in_bytes != tensor->element_size()) throw std::invalid_argument("incoherent element sizes in bytes");
      if ((int64_t)numel > tensor->numel()) throw std::invalid_argument("target numel is larger than tensor numel");
...
int at_save_image(tensor tensor, char *filename)
{
  PROTECT(
      ...
      if (tensor->device().type() != at::kCPU) throw std::invalid_argument("the input tensor has to be on cpu");
      if (sizes.size() != 3) throw std::invalid_argument("invalid number of dimensions, should be 3");
      int h = sizes[0];
      int w = sizes[1];
      int c = sizes[2];
      auto tmp_tensor = tensor->contiguous();
      void *tensor_data = tmp_tensor.data_ptr();
      if (ends_with(filename, ".jpg")) return stbi_write_jpg(filename, w, h, c, tensor_data, 90);
      if (ends_with(filename, ".bmp")) return stbi_write_bmp(filename, w, h, c, tensor_data);
      if (ends_with(filename, ".tga")) return stbi_write_tga(filename, w, h, c, tensor_data);
...
tensor atm_method(module m, char *method_name, tensor *tensors, int ntensors)
{
  PROTECT(
      ...
      if (!output.isTensor()) throw std::invalid_argument("method did not return a tensor");
      return new torch::Tensor(output.toTensor());)
  return nullptr;
}
    ...
    int ninputs)
{
  PROTECT(
      ...
      if (torch::jit::tracer::isTracing()) throw std::invalid_argument("cannot nest tracing calls");
